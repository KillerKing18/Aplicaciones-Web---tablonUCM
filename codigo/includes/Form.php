...
abstract class Form
{
     ...
     * se desea que se redirija al usuario, o un array con los mensajes que ha habido durante el procesamiento del formulario.
     */
     ...
     *
     * @param string[] $mensajes (opcional) Array con los mensajes de acierto o error de validación y/o procesamiento del formulario.
     *
     ...
     */
    private function generaFormulario($mensajes = array(), &$datos = array())
    {
        $html = $this->generaListaMensajes($mensajes);
     ...
     *
     * @param string[] $mensajes (opcional) Array con los mensajes de error de validación y/o procesamiento del formulario.
     *
     * @return string El HTML asociado a los mensajes de error.
     */
    private function generaListaMensajes($mensajes)
    {
        $html='';
        $numMensajes = count($mensajes);
        if (  $numMensajes == 2 ) {
            $html .= "<span id='spanMensajes' name='" . $mensajes[1] . "'><p id='mensajeValidacion'>" . $mensajes[0] . "</p></span>";
        } else if ( $numMensajes > 2 ) { //ERROR
            $html .= "<ul><li>";
            $html .= implode("</li><li>", $mensajes);
            $html .= "</li></ul>";
        }
        return $html;
    }
